...
void goalieLogic3(){
	Location puck = getPuckLocation();
	if(puck.y<getRobotPose().y+ROBOT_RADIUS) goToPosition2(Pose(XMIN+2*ROBOT_RADIUS,puck.y,0),getRobotPose(),false,false,1400);
	else if (puck.y>getRobotPose().y-ROBOT_RADIUS) goToPosition2(Pose(XMIN+2*ROBOT_RADIUS,puck.y,0),getRobotPose(),false,true,1400);
	else setMotors(0,0);
}